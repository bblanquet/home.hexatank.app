...
import { RedGame } from '../Model/Dialogues';
...
export class RedGameHook extends AbstractGameHook<GameBlueprint, Gameworld> {
	private _steps = 0;
	...
	protected Default(state: RuntimeState) {
		super.Default(state);
		state.Sentence = RedGame[0];
	}

	public SetNextSentence(): void {
		this._steps++;
		this.Update((e) => {
			e.Sentence = RedGame[this._steps];
		});

...
export class PeerHandler {
	...
	private static _handlers: { type: PacketKind; func: { (message: any): void } }[] = new Array<{
		type: PacketKind;
			...
			if (data.PlayerName !== this._player.Name) {
				if (data.sdp) {
					...
					this._connector.setRemoteDescription(new RTCSessionDescription(data.sdp)).then(() => {
						console.log(
							this._player.Name + ' set remote description ',
							this._connector.remoteDescription.type.toUpperCase()
						);

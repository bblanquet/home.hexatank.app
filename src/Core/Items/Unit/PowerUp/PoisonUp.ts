...
export class PoisonUp extends Up {
	...
	public Update(): void {
		...
		if (this._timer.IsElapsed()) {
			this.Vehicle.SetDamage(Math.abs(this._upCalculator.GetPoison(this.GetCurrentEnergy())));
		}
	}

	protected OnEnergyChanged(previousEnery: number, energy: number): void {
		if (previousEnery === 0 && 0 < energy) {
			this.Animation = new UpAnimation(this.Vehicle, SvgArchive.poisonUp, SvgArchive.poisonUpR);

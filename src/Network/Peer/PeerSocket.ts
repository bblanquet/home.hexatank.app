...
export abstract class PeerSocket {
	...
	protected ReceivePacket(event: MessageEvent): void {
		...
		if (packet.Recipient === PeerSocket.All() || packet.Recipient === this.Owner) {
			...
			message.Content = (packet as any).Content ?? null;
	...
	private LogMessage(message: INetworkMessage) {
		const data = `[${message.Emitter} -> ${this.Owner}] ${this.Connection.remoteDescription.type.toUpperCase()} <<<`;
		console.log(data);
	}
	...
	private CreatePing() {
		...
		this.PeerPing.PingReceived.On((obj: any, latency: string) => {
			...
			if(this._lastConnectionStatus && this._lastConnectionStatus.IsNotConnected()){ //code for firefox

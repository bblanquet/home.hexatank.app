import { TestHelper } from './../src/Core/Utils/TestHelper';
import 'mocha';
import { expect } from 'chai';
import { HexAxial } from '../src/Core/Utils/Coordinates/HexAxial';
import { CeilProperties } from '../src/Core/Ceils/CeilProperties';
import { AStarNode } from '../src/Core/Ia/AStarNode';
import { HexagonalMapBuilder } from '../src/Core/Setup/Builder/HexagonalMapBuilder';

describe('Astar',()=>{
    before(()=>{
        TestHelper.Init();
         var playgroundMaker = new HexagonalMapBuilder();
         playgroundMaker.Build(8).forEach(ceil=>
         {
    ...
    it('Get Distance from 52 to 43',()=>{
        ...
        var goalNode = new AStarNode<CeilProperties>(goalCeil);
        var distance = starNode.GetEstimatedCost(goalNode);
        expect(Math.round(startCeil.BoundingBox.Width)).to.equals(Math.round(distance));
    });
